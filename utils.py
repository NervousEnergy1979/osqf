...
def calculate_drawdown(
    ...
    """Calculate the rolling cumulative drawdown of returns.

    Args:
        returns: Returns stream. Shape is Tx1

    Returns:
        The drawdown for each point in time. Shape is Tx1.
    """
    cum_rets = returns.add(1).cumprod()
    cum_max = cum_rets.expanding().max()
    drawdown = (cum_rets - cum_max) / cum_max
    return drawdown


def calculate_annualised_returns(
    returns: pd.Series,
    scale: int,
    geometric: bool,
) -> float:
    """Calculate the average annualised returns.

    Args:
        returns: Returns stream. Shape is Tx1
        scale: Frequency of returns corresponding to
            number of periods in 1 year. e.g. 365 for daily, 8,760 for hourly.
        geometric: If True, use compounded (geometric) average, else arithmetic

    Returns:
        The average annualised returns.
    """
    if geometric:
        out = 100 * (returns.add(1).prod() ** (scale / len(returns)) - 1)
    else:
        out = 100 * scale * returns.mean()
    return out


def calculate_annualised_volatility(returns: pd.Series, scale: int) -> float:
    """Calculate the annualised volatility of returns.
    ...
    """
    out = 100 * np.sqrt(scale) * returns.std()
    return out
...
def calculate_sharpe_ratio(
    ...
    Args:
        returns: Returns stream. Shape is Tx1
        scale: Frequency of returns corresponding to
            number of periods in 1 year. e.g. 12 for monthly, 365 for daily, 8,760 for hourly.
        geometric: If True, use geometric returns, else arithmetic.

    Returns:
        The annualised Sharpe ratio.
    """
    if geometric:
        num = calculate_annualised_returns(returns=returns, scale=scale, geometric=True)
        denom = calculate_annualised_volatility(returns=returns, scale=scale)
    else:
        num = np.sqrt(scale) * returns.mean()
        denom = returns.std()
